...
"""
 ...
 dT/dlai as function of snow cover and solar radiation
...
def read_snowcoverLAI(yr_,mth_):
  ...
  snowcover=MV.masked_less(snowcover[::-1,:],1)#mask snowcover <1%
  msk=(snowcover+999)/(snowcover+999)
  # read SWdown
  f=cdms.open('/ESS_EarthObs/REANALYSIS/ERA5/radiative/SWdown_005deg_%i.nc'%(yr_))
  temp=f('ssrd',cdms.timeslice(mth_-1,mth_),lat=latrange,lon=lonrange,squeeze=1)*msk/86400. # J/m2 to W/m2
  f.close()
  msk=(temp+999)/(temp+999)*msk
  ...
  f.close()
  snowcover=snowcover*msk
  return snowcover,lai_,temp,msk
...
def andiff(yd,yf):
  for mm in range(1,13,1):
    ...
    msk=msk1*msk2
    del msk1,msk2
    abstemp=(temp1+temp2)/2.#MV.where(temp1>temp2,temp1,temp2)
    latent=(latent1+latent2)/2.#MV.where(latent1>latent2,latent1,latent2)
    dlai=lai2-lai1
    del lai1,lai2,temp1,temp2,latent1,latent2
    dt_dlai=MV.where(dlai==0,1.e20,dt/dlai)
    del dlai, dt
    dt_dlai=MV.masked_greater(dt_dlai,20) # normaly not needed but just to make sure in case of very low dlai => large dt/dlai
    dt_dlai=MV.masked_less(dt_dlai,-20)   # normaly not needed but just in case of very low dlai => very low dt/dlai
    ari=area[:,:]
...
f=cdms.open('Snow_SWdown_sensitivity.nc','w')
